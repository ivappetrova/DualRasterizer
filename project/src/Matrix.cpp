...
namespace dae {
	...
	Matrix Matrix::CreateLookAtLH(const Vector3& origin, const Vector3& forward, const Vector3& up)
	{
		...
		const float TX{ -Vector3::Dot(x, origin) };
		const float TY{ -Vector3::Dot(y, origin) };
		const float TZ{ -Vector3::Dot(z, origin) };

		return Matrix{ Vector4{ x.x, x.y, x.z, 0 },
					   Vector4{ y.x, y.y, y.z, 0 },
					   Vector4{ z.x, z.y, z.z, 0 },
					   Vector4{ TX,  TY,  TZ,  1 } };
	}

	Matrix Matrix::CreatePerspectiveFovLH(float fov, float aspect, float zn, float zf)
	{
		const float Y_SCALE = 1.0f / tanf(fov / 2.0f);
		const float X_SCALE = Y_SCALE / aspect;
		const float Z_RANGE = zf / (zf - zn);

		Matrix projectionMatrix;

		projectionMatrix[0] = Vector4(X_SCALE, 0.0f, 0.0f, 0.0f);
		projectionMatrix[1] = Vector4(0.0f, Y_SCALE, 0.0f, 0.0f);
		projectionMatrix[2] = Vector4(0.0f, 0.0f, Z_RANGE, 1.0f);
		projectionMatrix[3] = Vector4(0.0f, 0.0f, -zn * Z_RANGE, 0.0f);
